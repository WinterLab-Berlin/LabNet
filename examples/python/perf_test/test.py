    ...
    for r in range(cycles):
        ...
        while True:
            ...
            if isinstance(mes, LabNetServer.DigitalInState) and mes.state == turn_on:
                el = (time.perf_counter_ns() - t0) / 1_000_000
                set_read_latencies.append(el)
                break

        turn_on = not turn_on

    median = np.percentile(set_read_latencies, 50)
    p2_5 = np.percentile(set_read_latencies, 2.5)
    p25 = np.percentile(set_read_latencies, 25)
    p75 = np.percentile(set_read_latencies, 75)
    p97_5 = np.percentile(set_read_latencies, 97.5)
    mean = np.mean(set_read_latencies)
    std = np.std(set_read_latencies)

    print(f"\rmean: {mean:.2f} std dev: {std:.2f} median: {median:.2f} p25: {p25:.2f} p75: {p75:.2f} p2.5: {p2_5:.2f} p97.5: {p97_5:.2f}")

print("save latencies")
f = open("latencies.csv", "w")
f.write("IdTest;SetTest;SetAndRead\n")
for r in range(cycles):
    f.write(f"{id_latencies[r]:.2f};{set_latencies[r]:.2f};{set_read_latencies[r]:.2f}\n")
