        ...
        init_state_
            .event(self_box_,
                [this](const mhood_t<start>) {
                    std::cout << "set and read digital out test" << std::endl;
            ...
            .event(self_box_,
                [this](const std::shared_ptr<LabNetProt::Server::DigitalOutInitResult> res) {
                    if (res->is_succeed())
                    {
                        ...
                        std::shared_ptr<LabNetProt::Client::GpioWiringPiInitDigitalIn> dig_in = std::make_shared<LabNetProt::Client::GpioWiringPiInitDigitalIn>();
                        dig_in->set_pin(23);
                        dig_in->set_is_inverted(false);
                        client_->SendMessage(dig_in, LabNetProt::Client::GPIO_WIRINGPI_INIT_DIGITAL_IN);
                    }
                    else
                    {
            ...
            .event(self_box_,
                [this](const std::shared_ptr<LabNetProt::Server::DigitalInState> res) {
