...
void ResourceRequestHelper::so_define_agent()
{
    so_subscribe_self()
        .event([this](const so_5::mhood_t<DigitalInput> msg) {
            resources::resource res = resources::wiring_to_resource(msg->pin_h);
        ...
        .event([this](const so_5::mhood_t<resources::reserve_resources_reply> msg) {
            ...
            if (inp_it != _inputs.end())
            {
                if (msg->result)
                {
                    resources::resource res = resources::wiring_to_resource(inp_it->second->pin_h);
            ...
            else
            {
                ...
                if (out_it != _outputs.end())
                {
                    if (msg->result)
                    {
                        resources::resource res = resources::wiring_to_resource(out_it->second->pin_h);
                        _acquired.push_back(res);
                    }
