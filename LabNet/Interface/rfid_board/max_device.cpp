...
{
    MAXDevice::MAXDevice(Logger logger, so_5::mbox_t mbox)
        : _logger(logger)
        , _parentMbox(mbox)
        ...
        while (counter > 0)
        {
            ...
            if (max_uart_RFIDCounter[cspin][uart] == 14)
            {
                if ((max_uart_RFIDFifo[cspin][uart][0] == 0x02) && (max_uart_RFIDFifo[cspin][uart][12] == 0x0a) && (max_uart_RFIDFifo[cspin][uart][13] == 0x03))
                {
                    ...
                    so_5::send<stream_messages::NewDataFromPort>(_parentMbox, Interfaces::RFID_BOARD, 4 * cspin + uart + 1, data, std::chrono::high_resolution_clock::now());
                }
                else
                {
                }
    ...
    void MAXDevice::set_phase_matrix(uint32_t antenna_phase1, uint32_t antenna_phase2, uint32_t phase_duration)
    {
    ...
    void MAXDevice::switch_phase_matrix()
    {
        using namespace std::chrono;
    ...
    void MAXDevice::read_all_and_set_antenna()
    {
        uint8_t cnt1, cnt2;
        for (cnt1 = 0; cnt1 < 8; cnt1++)
            for (cnt2 = 0; cnt2 < 4; cnt2++)
