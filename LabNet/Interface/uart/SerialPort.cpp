...
void SerialPort::send_data(std::shared_ptr<LabNetProt::Client::UartWriteData> data)
{
    so_5::send<std::shared_ptr<LabNetProt::Client::UartWriteData>>(_sendToPortBox, data);
}
    ...
    receive(from(ch).handle_all(),
        [&](std::shared_ptr<LabNetProt::Client::UartWriteData> data) {
            if (_isActive)
            {
                ...
                {
                    serialPutchar(_portHandler, data->data()[i]);
                }
